...
$request=$user ? 'user/name' : ($uid ? 'user/id' : 'date');
$init=$user ? $user : ($uid ? $uid : ($date ? date('Y-m-d',strtotime($date)) : date('Y-m-d',time()-86400)));
$req_type=$user ? 'User' : ($uid ? 'Uid' : 'Date');
